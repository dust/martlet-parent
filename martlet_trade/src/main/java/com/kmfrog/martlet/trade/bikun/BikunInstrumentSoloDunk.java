...
public class BikunInstrumentSoloDunk extends InstrumentSoloDunk{
	
	BikunApiRestClient client;
	private final AtomicLong lastOrder;
	
	public BikunInstrumentSoloDunk(Instrument instrument, Source src, TrackBook trackBook, Provider provider,
			BikunApiRestClient client, Param param) {
	...
	public void placeHedgeOrder(long price, long spreadSize, IOrderBook book) {
        ...
        if(now - lastOrder.get() > avgSleepMillis) {
        	provider.submitExec(new BikunHedgeOrderExec(source, instrument, price, spreadSize, vMin, vMax, avgSleepMillis,
