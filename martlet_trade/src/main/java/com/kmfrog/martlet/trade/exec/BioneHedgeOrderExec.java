...
public class BioneHedgeOrderExec extends Exec{
	
	static Logger logger = LoggerFactory.getLogger(BioneHedgeOrderExec.class);
	
	BioneApiRestClient client;
	Provider provider;
    long price;
    long spread;
    TrackBook trackBook;
	...
	public BioneHedgeOrderExec(Source src, Instrument instrument, long price, long spread, long vMin, long vMax,
            int avgSleep, BioneApiRestClient client, TrackBook trackBook, Provider provider) {
    ...
    public void run() {
        try {
            ...
            if (isSellFirst) {
            	Long sellOrderId = client.limitSell(instrument.asString(), quantityStr, priceStr);
                if (sellOrderId != null) {
                    if (logger.isInfoEnabled()) {
                    ...
                    Long buyOrderId = client.limitBuy(instrument.asString(), quantityStr, priceStr);
                    if (buyOrderId != null) {
